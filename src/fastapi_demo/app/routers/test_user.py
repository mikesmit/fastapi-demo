...
def optional_auth_override(creds:HTTPAuthorizationCredentials | None = Depends(HTTPBearer(auto_error=False))):
    '''
...
def createdUser(client:TestClient)->Any:
    response = client.post("/user", json={
        ...
        "authorization":'Bearer {"sub":"github|whatever"}'
    })
...
def test_create_user(client:TestClient):
    ...
    assert response.json() == {
        'auth0_sub': 'github|whatever', 'id': 1, 'username': 'test_username'
    }
...
def test_create_user_wrote_sub(client:TestClient):
    response = client.post("/user", json={
        ...
        "authorization":'Bearer {"sub":"some_other_id"}'
    })
...
def test_delete_user_wrong_auth(client:TestClient, createdUser):
    response = client.delete(
        f"/user/{createdUser['id']}",
        headers={
            "authorization":f'Bearer {{"sub":"not_right"}}'
        }
    )
