...
def create_router(session_dependency:SessionGeneratorFactory):
    ...
    async def create_houshold(item:HouseholdCreate,
                              session:Annotated[Session, Depends(session_dependency)])->Household:
    ...
    async def delete_household(id:int,
                               session:Annotated[Session, Depends(session_dependency)])->None:
